...
namespace Tokenio.Rpc
{
    ...
    public class AsyncClientAuthenticator : Interceptor
    {
            ...
            AsyncUnaryCallContinuation<TRequest, TResponse> continuation)
        {
            ...
            CustomerTrackingMetadata customer = authentication.CustomerTrackingMetadata;
            if (!string.IsNullOrEmpty(customer.IpAddress))
            {
                metadata.Add("token-customer-ip-address", customer.IpAddress);
            }
            if (!string.IsNullOrEmpty(customer.GeoLocation))
            {
                metadata.Add("token-customer-geo-location", customer.GeoLocation);
            }
            if (!string.IsNullOrEmpty(customer.DeviceId))
            {
                metadata.Add("token-customer-device-id", customer.DeviceId);
            }
